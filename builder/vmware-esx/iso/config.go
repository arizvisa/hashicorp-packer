...
func NewConfig(raws ...interface{}) (*Config, []string, error) {
	...
	// If SkipCompaction is not specified but there's no disk type id,
	// then we have no choice but to skip it.
	if !c.SkipCompaction && c.DiskTypeId == "" {
		c.SkipCompaction = true
	}

	// If no disk type id was specified, then we need to default it
	// to "zeroedthick" so that it matches the previous logic of the esx
	// driver
	if c.DiskTypeId == "" {
		c.DiskTypeId = "zeroedthick"
	}

	// If we're using a disk type id of "think", then we can't compact the
	// image. This matches the previous logic of the esx driver.
	...
	// Remote configuration validation for ESX
	if c.RemoteHost == "" {
		errs = packer.MultiErrorAppend(errs,
